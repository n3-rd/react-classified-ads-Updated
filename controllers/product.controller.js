...
class APIFeatures {
  ...
  sorting() {
    if (this.queryString.sort) {
      ...
      // Add sorting by featured (descending order) as the primary sorting field
      this.query = this.query.sort('-featured ' + sortBy);
    } else {
      // Default sorting: Featured first, then createdAt
      this.query = this.query.sort('-featured -createdAt');
    }
...
const productCtrl = {
  getProduct: async (req, res) => {
    try {
      const features = new APIFeatures(Products.find({ checked: true }), req.query)
        .filtering()
        .sorting()
        .pagination();
  ...
  try {
    ...
    // Generate product_id (example using a timestamp)
    const product_id = "PROD" + Date.now().toString(36);
  ...
  updateProduct: async (req, res) => {
    try {
      ...
      // Update the product with the new data and set checked to false
